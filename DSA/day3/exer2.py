...
class Stack:
    ...
    #You can use the below __str__() to print the elements of the 
    #DS object while debugging
    def __str__(self):
        msg=[]
        index=self.__top
        while(index>=0):
...
class Box:
    ...
    def group_balls(self):
        for i in range(self.ball_stack.get_max_size()):
            data=self.ball_stack.pop()
            if(data.get_manufacturer()=="Penn"):
                self.manufacturer1_stack.push(data)
            else:
                self.manufacturer2_stack.push(data)
